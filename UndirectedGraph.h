...
public:
  ...
  bool createEdge(string id1, string id2, TE w){
    if(this->vertexes.find(id1)!= this->vertexes.end() && this->vertexes.find(id2)!= this->vertexes.end()){
      Edge<TV, TE>* arista1 = new Edge<TV, TE>;
      Edge<TV, TE>* arista2 = new Edge<TV, TE>;
      arista1->vertexes[0] = this->vertexes[id1];
      arista1->vertexes[1] = this->vertexes[id2];
      arista2->vertexes[0] = this->vertexes[id2];
      arista2->vertexes[1] = this->vertexes[id1];
      arista1->weight = w;
      arista2->weight = w;
      this->n_edges++;
      this->vertexes[id1]->edges.push_back(arista1);
      this->vertexes[id2]->edges.push_back(arista2);
      return true;
    }
    else{
      return false;
	...
	TE &operator()(string start, string end){
		...
		for(auto& vertex: this->vertexes){
			for(auto& edge: vertex.second->edges){
				if((edge->vertexes[0] == this->vertexes[start] && edge->vertexes[1] == this->vertexes[end]) || (edge->vertexes[0] == this->vertexes[end] && edge->vertexes[1] == this->vertexes[start])){
					val = edge->weight;
				}
			}
		}
